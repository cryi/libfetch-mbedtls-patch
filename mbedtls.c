...
int init_cacert(int verbose)
{
    if (cacert == NULL) {
        ...
        int ret = mbedtls_x509_crt_parse_file(cacert, "cacert.pem"); //cacertPath == NULL ? "cacert.pem" : cacertPath);
...
int init_clicert(int verbose)
{
    ...
    if (clicert == NULL)
    {
        ...
        if (clicertPath != NULL && clikeyPath != NULL)
        {
            ...
            ret = mbedtls_pk_parse_keyfile(clikey, clikeyPath, clikeyPass);
...
int fetch_mbedtls(conn_t *conn, const struct url *URL, int verbose, int permissive)
{
    ...
    conn->buf_events = 0;
    mbedtls_ssl_set_bio(conn->ssl, &conn->sd, mbedtls_net_send, mbedtls_net_recv, NULL);
